...
class BiAgendaController {
    ...
    try {
      ...
      if (CONTROLLED_CODES.has(error?.code)) {
        return res.status(400).json({
          ...
          error: "Filtros do BI de agenda invalidos.",
          success: false,
        });
      }
      return next(error);
  ...
  getService() {
    if (!this.service || typeof this.service.getAnalytics !== "function") {
      throw Object.assign(new TypeError("BiAgendaController requires BiAgendaService."), {
        code: "BI_AGENDA_SERVICE_INVALID",
      });
    }
    return this.service;
  }
...
module.exports = { BiAgendaController };
