...
const FINANCIAL_KPIS_SQL = `
  ...
  WHERE c.ativo = 1 AND (? IS NULL OR EXISTS (SELECT 1 FROM j12_unidades u WHERE CAST(u.id AS CHAR) = ? AND (u.nome = c.unidade OR CAST(u.id AS CHAR) = c.unidade)))
`;
