...
const SELECT_BY_LEAD = `SELECT id,lead_id,unit_id,person_id,person_profile_id,status,converted_by,converted_at,idempotency_key,metadata_json FROM ${TABLE} WHERE lead_id=? AND unit_id=? LIMIT 1`;
const LOCK_LEAD =
  "SELECT id FROM crm_leads WHERE id=? AND unit_id=? AND deleted_at IS NULL FOR UPDATE";
const INSERT = `INSERT INTO ${TABLE} (id,lead_id,unit_id,person_id,person_profile_id,status,converted_by,converted_at,idempotency_key,metadata_json) VALUES (?,?,?,?,?,?,?,?,?,?)`;

class MySqlCrmLeadStudentConversionRepository {
  ...
  async create(input = {}) {
    return this.transaction(async (connection) => {
      ...
      try {
        ...
        if (!recovered) throw repositoryError("CRM_LEAD_STUDENT_CONVERSION_CONFLICT");
      ...
      if (!created) throw repositoryError("CRM_LEAD_STUDENT_CONVERSION_FAILED");
