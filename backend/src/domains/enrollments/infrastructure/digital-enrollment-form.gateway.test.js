...
const INVITATION = { enrollmentId: "enrollment-1", invitationId: "invitation-1", unitId: "1" };

test("getForm returns resumable canonical data", async () => {
  const { gateway } = fixture();
  const form = await gateway.executeDigitalEnrollmentOperation({
...
test("partial writes persist each allowlisted step and increment revision", async () => {
  ...
  let form = await gateway.executeDigitalEnrollmentOperation({
    command: { fields: { name: "Responsavel Atualizado" }, revision: 1 },
    invitation: INVITATION,
  ...
  form = await gateway.executeDigitalEnrollmentOperation({
    command: { fields: { birthDate: "2014-05-06", name: "Aluno Atualizado" }, revision: 2 },
    invitation: INVITATION,
    operation: "updateStudent",
  });
  assert.equal(form.student.birthDate, "2014-05-06");
  form = await gateway.executeDigitalEnrollmentOperation({
    command: { fields: { city: "Goiania", zipCode: "74000-000" }, revision: 3 },
    invitation: INVITATION,
    operation: "updateAddress",
  });
  assert.equal(form.address.city, "Goiania");
  form = await gateway.executeDigitalEnrollmentOperation({
    command: { fields: {}, revision: 4 },
    invitation: INVITATION,
    operation: "updateAdditionalInformation",
  });
...
test("advance remains sequential through DOCUMENTS", async () => {
  const { gateway, state } = fixture();
  for (const [revision, targetStep] of [
    [1, "STUDENT_DATA"],
    [2, "ADDRESS"],
    [3, "ADDITIONAL_INFORMATION"],
    [4, "DOCUMENTS"],
  ]) {
    await gateway.executeDigitalEnrollmentOperation({
      command: { fields: { targetStep }, revision },
      invitation: INVITATION,
...
test("step jumps remain invalid", async () => {
  ...
  await assert.rejects(
    () =>
      gateway.executeDigitalEnrollmentOperation({
        command: { fields: { targetStep: "ADDRESS" }, revision: 1 },
        invitation: INVITATION,
        operation: "advanceStep",
      }),
...
test("advance preserves optimistic revision conflicts before REVIEW blocking", async () => {
  const { gateway, state } = fixture({
    progress: { currentStep: "DOCUMENTS", revision: 5, status: "IN_PROGRESS" },
  });
  await assert.rejects(
    () =>
      gateway.executeDigitalEnrollmentOperation({
        command: { fields: { targetStep: "REVIEW" }, revision: 4 },
        invitation: INVITATION,
        operation: "advanceStep",
      }),
...
test("stale revision returns controlled conflict without merging", async () => {
  ...
  await assert.rejects(
    () =>
      gateway.executeDigitalEnrollmentOperation({
        command: { fields: { name: "Stale" }, revision: 2 },
        invitation: INVITATION,
        operation: "updateStudent",
      }),
    { code: "DIGITAL_ENROLLMENT_PROGRESS_CONFLICT", statusCode: 409 },
...
for (const [name, override] of [
  ...
  ["enrollment without unit", { enrollment: { unitId: null } }],
  ["unit mismatch", { enrollment: { unitId: "2" } }],
  ...
  test(`fails closed when ${name}`, async () => {
    ...
    await assert.rejects(
      () =>
        gateway.executeDigitalEnrollmentOperation({ invitation: INVITATION, operation: "getForm" }),
      { code: "DIGITAL_ENROLLMENT_OWNERSHIP_NOT_AVAILABLE" },
    );
  });
}

test("fails closed when invitation has no unit ownership", async () => {
  const { gateway } = fixture();

  await assert.rejects(
    () =>
      gateway.executeDigitalEnrollmentOperation({
        invitation: { enrollmentId: "enrollment-1", invitationId: "invitation-1" },
        operation: "getForm",
      }),
    { code: "DIGITAL_ENROLLMENT_OWNERSHIP_NOT_AVAILABLE" },
  );
...
function fixture(overrides = {}) {
  ...
  const state = {
    ...
    people: {
      "responsible-1": {
        address: {},
        contact: { email: "r@example.test", phone: "62999990000" },
        name: { fullName: "Responsavel" },
      },
  ...
  const queryRunner = async (sql, params) => {
    ...
    const columns = [...sql.slice(0, sql.indexOf(" WHERE ")).matchAll(/([a-z_]+) = \?/g)].map(
      (match) => match[1],
    );
  ...
  const context = {
    ...
    personProfileRepository: {
      async findById(id) {
        return id === "responsible-profile-1"
          ? { personId: "responsible-1" }
    ...
    digitalEnrollmentProgressRepository: {
      ...
      async updateIfRevisionMatches(input) {
        ...
        if (!state.progress || state.progress.revision !== input.expectedRevision) {
  ...
  return {
    gateway: new DigitalEnrollmentFormGateway({
      transactionRunner: async (callback) => callback(context),
    }),
    state,
  };
}
function applyColumn(person, column, value) {
  ...
  if (path.length === 1) person[path[0]] = value;
  else person[path[0]][path[1]] = value;
}
...
test("read-only advance inspection validates revision, step and REVIEW target", async (t) => {
  await t.test("revision", async () => {
    ...
    await assert.rejects(
      () =>
        gateway.inspectDigitalEnrollmentAdvance({
          command: { fields: { targetStep: "REVIEW" }, revision: 4 },
          invitation: INVITATION,
        }),
  ...
  await t.test("current step", async () => {
    const { gateway } = fixture({
      progress: { currentStep: "ADDITIONAL_INFORMATION", revision: 5, status: "IN_PROGRESS" },
    });
    await assert.rejects(
      () =>
        gateway.inspectDigitalEnrollmentAdvance({
      ...
      { code: "DIGITAL_ENROLLMENT_INVALID_STEP" },
    );
  });
  await t.test("target step", async () => {
    ...
    await assert.rejects(
      () =>
        gateway.inspectDigitalEnrollmentAdvance({
          command: { fields: { targetStep: "DOCUMENTS" }, revision: 5 },
          invitation: INVITATION,
        }),
      { code: "DIGITAL_ENROLLMENT_INVALID_STEP" },
    );
  ...
  for (const [name, override] of [
    ...
    await t.test(name, async () => {
      ...
      await assert.rejects(
        () =>
          gateway.inspectDigitalEnrollmentAdvance({
            command: { fields: { targetStep: "REVIEW" }, revision: 1 },
            invitation: INVITATION,
          }),
        { code: "DIGITAL_ENROLLMENT_OWNERSHIP_NOT_AVAILABLE" },
      );
